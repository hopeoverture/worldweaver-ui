...
-- Entities: index for world_id and updated_at DESC (remove CONCURRENTLY for migrations)
CREATE INDEX IF NOT EXISTS idx_entities_world_updated 
...
CREATE INDEX IF NOT EXISTS idx_entities_world_template_updated 
  ON entities(world_id, template_id, updated_at DESC);

-- Example: Composite index for templates by world_id, category, and name
CREATE INDEX IF NOT EXISTS idx_templates_world_category_name 
