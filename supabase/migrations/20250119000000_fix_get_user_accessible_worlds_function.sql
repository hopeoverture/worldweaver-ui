-- Fix get_user_accessible_worlds function to use existing column names
-- The function was referencing display_name which doesn't exist, should use full_name
...
BEGIN
  ...
  SELECT DISTINCT
    ...
    END as user_role,
    -- Use full_name instead of display_name (which doesn't exist)
    COALESCE(p.full_name, p.email, 'Unknown') as owner_name,
...
COMMENT ON FUNCTION get_user_accessible_worlds(UUID) IS 'Returns all worlds accessible to a user (owned or member worlds) with role and owner information';