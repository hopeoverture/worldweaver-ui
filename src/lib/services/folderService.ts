...
export class FolderService {
   ...
   */
  async createFolder(worldId: string, data: { name: string; description?: string; color?: string; kind?: 'entities' | 'templates'; parentFolderId?: string; [key: string]: unknown }, userId: string): Promise<Folder> {
    // Access check
    const world = await worldService.getWorldById(worldId, userId);
    if (!world) throw new Error('World not found or access denied');
