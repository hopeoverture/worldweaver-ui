...
import { VirtualEntityGrid } from './VirtualEntityGrid';
...
export function EntityGrid({ entities, onCreateEntity, onDragStart, worldId, onDelete }: EntityGridProps) {
  ...
  const emptyStateWithButton = onCreateEntity ? (
  ...
  const renderRegular = (items: Entity[]) => (
    <>
  ...
  const renderVirtual = (items: Entity[]) => (
    <VirtualEntityGrid
      entities={items}
